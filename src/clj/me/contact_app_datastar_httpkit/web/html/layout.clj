(ns me.contact-app-datastar-httpkit.web.html.layout
...
(defn base-layout [title content]
  [:html {:lang "en"}
   ...
   [:body {:class "bg-background text-white min-h-screen flex items-center justify-center"
           :data-on-load (cc/raw "history.replaceState('/', '', '/')")
           :data-on-popstate__window (cc/raw "evt.state === '/' || evt.state === '' ? window.location.href = '/' : @get(evt.state); console.log(evt.state)")}
    [:main {:class "w-full max-w-xl p-6 bg-surface rounded-xl shadow-lg space-y-6"}
